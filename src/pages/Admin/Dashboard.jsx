...
const Dashboard = () => {
  ...
  const report_icon = 'https://res.cloudinary.com/dspuitf5t/image/upload/v1732626973/dash_report_klo8b5.svg'

  const {aToken, dashData, getDashData, cancelAppointment, cancelReport} = useContext(AdminContext)
  ...
  return dashData && (
    <div className='m-5'>
        <div className='flex flex-wrap gap-3'>
          ...
          <div onClick={()=>navigate('/all-reports')} className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
            <div className='w-14 bg-[#F6F8FF] flex items-center justify-center rounded py-2'>
              <img className='w-10' src={report_icon} alt="" />
            </div>
            <div>
              <p className='text-xl font-semibold text-gray-600'>{dashData.reports}</p>
        ...
        <div className='bg-white'>
          <div className='flex items-center gap-2.5 px-4 py-4 mt-10 rounded-t border'>
            <img src={assets.list_icon} alt="" />
            <p className='font-semibold'>Latest Reports</p>
          </div>
          <div className='pt-4 border border-t-0'>
            {
              dashData.latestReports.map((item, index)=>(
                <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
                  <img className='rounded-full w-10' src={item.userData.image} alt="" />
                  <div className='flex-1 text-sm'>
                    <p className='text-gray-800 font-medium'>{item.userData.name}</p>
                    <p className='text-gray-600'>{slotDateFormat(item.appliedDate)}</p>
                  </div>
                  { 
                    item.cancelled 
                    ? <p className='text-red-400 text-xs font-medium'>Cancelled</p>
